...
export function MasterPlatformPanel({ overview, loading, error }: MasterPlatformPanelProps) {
  ...
  const renderStatusList = (byStatus: Record<string, number>) => {
    ...
    if (entries.length === 0) return <p className="master-hint">Nenhuma assinatura.</p>
    return (
      <ul className="master-status-list">
        {entries.map(([status, n]) => (
          <li key={status}>
            <span>{STATUS_LABEL[status] ?? status}</span>
            <span className="master-status-value">{n}</span>
          </li>
        ))}
      </ul>
    )
  ...
  return (
    <div className="master-dashboard">
      <section className="master-section">
        <p className="master-section-title">Assinaturas da plataforma (profissionais)</p>
        {renderStatusList(o.platform_subs_by_status)}
      </section>

      <section className="master-section">
        <p className="master-section-title">Assinaturas de comunidade</p>
        {renderStatusList(o.community_subs_by_status)}
      </section>

      <section className="master-section">
        <p className="master-section-title">Trials</p>
        <div className="master-metrics-grid">
          <div className="master-metric-tile">
            <p className="master-metric-tile-value">{o.trials_ending_7d}</p>
            <p className="master-metric-tile-label">Trials terminando em 7 dias</p>
          </div>
        </div>
      </section>

      <section className="master-section">
        <p className="master-section-title">Split de receita</p>
        <div className="master-metrics-grid">
          <div className="master-metric-tile">
            <p className="master-metric-tile-value">
              {o.split_professional_percent != null ? `${o.split_professional_percent}%` : '—'}
            </p>
            <p className="master-metric-tile-label">Profissional</p>
          </div>
          <div className="master-metric-tile">
            <p className="master-metric-tile-value">
              {o.split_circula_percent != null ? `${o.split_circula_percent}%` : '—'}
            </p>
            <p className="master-metric-tile-label">Círcula</p>
          </div>
        </div>
      </section>

      <section className="master-section">
        <p className="master-section-title">Planos</p>
        {o.plans.length === 0 ? (
          <p className="master-hint">Nenhum plano cadastrado.</p>
        ) : (
          <ul className="master-status-list">
            {o.plans.map((plan) => (
              <li key={plan.id}>
                <span>
                  {plan.name} <span className="master-status-value">({plan.subject})</span>
                  {!plan.is_active && ' — inativo'}
                </span>
                <span className="master-status-value">
